...
void CFlatHeaderCtrl::DrawItem(CDC* pDC, CRect rect, LPHDITEM lphdi, BOOL bSort, BOOL bSortAscending)
{
	...
	case HDF_CENTER:
		...
		iWidth=DrawBitmap(pDC, rect, lphdi, pBitmap, &BitmapInfo, TRUE);
	...
	case HDF_RIGHT:
		...
		{
			iWidth=DrawBitmap(pDC, rect, lphdi, pBitmap, &BitmapInfo, FALSE);
			rect.left += (iWidth) ? iWidth+m_iSpacing:0;
		}
...
INT CFlatHeaderCtrl::DrawBitmap(
	CDC* pDC, 
	CRect rect, 
	LPHDITEM /*lphdi*/, 
	CBitmap* pBitmap, 
...
LRESULT CFlatHeaderCtrl::OnInsertItem(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	WPARAM wItems = m_arrayHdrItemEx.GetSize();
